...
- `src/services/imageService.js` — WebP featured images (per-blog size, see `imageSpecForBlog`; e.g. 1200×630). `saveTempImage()` is the orchestrator (NOT `generateFeaturedImage()`, which is now only the SVG-gradient fallback). Source order: **Codex banner** (primary, all blogs) → **OpenAI** (if `OPENAI_API_KEY`; `openai-banner` for banner blogs like LaraCopilot, else `openai`) → per-blog free sources (ViitorCloud `preferStock`: Pexels→Unsplash→Pollinations; others: Pollinations→Pexels→Unsplash) → **SVG gradient** (always works). The source actually used is recorded in `jobs.image_source` (`codex-banner`/`openai-banner`/`openai`/`pollinations`/`pexels`/`unsplash`/`gradient`). The blog logo is composited onto every source EXCEPT `codex-banner`/`openai-banner` (those already overlay the real `logo.svg` in-place).
- `src/services/codexImageService.js` — `generateCodexBanner()`: the primary image source. Runs `codex exec` (OpenAI Codex CLI's built-in `image_gen`, uses the user's ChatGPT login — no API key) to generate a per-brand banner, then composites the real per-domain `logo.svg` (deterministic, never AI-drawn). Per-brand visuals in the `BRANDS` map (keyed by blog slug/name/context_path). Returns a WebP Buffer or `null`. Disable with `CODEX_IMAGE_DISABLED=1`; auto-disables when the `codex` binary/login is unavailable (e.g. headless servers).
...
2. `seomachineService.loadMethodology(root)` reads the **live** ordered 4-command pipeline from `.claude/commands/` — `research.md` → `write.md` → `optimize.md` → `scrub.md` (the `METHODOLOGY_COMMANDS` array) — concatenated as phases the model must run in order. Reads live, so upstream improvements flow in automatically. Do NOT copy command text into code.
   - **Disabled rules files**: `loadRulesFile()` (in `claudeService.js`) skips any basename in `DISABLED_RULES_FILES` (env-overridable; default `vc_blog_generation.md`) — so it's never injected by either engine. Per user directive, ViitorCloud relies on the 4-command methodology + its `context/`, not a per-blog rules file.
